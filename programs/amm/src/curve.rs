...
pub fn get_delta_amount_base_unsigned(
    ...
    require!(result <= U256::from(u64::MAX), AmmError::MathOverflow);
    Ok(result.try_into().map_err(|_| AmmError::TypeCastFailed)?)
}
...
pub fn get_delta_amount_base_unsigned_unchecked(
    ...
    let result = mul_div_u256(numerator_1, numerator_2, denominator, round)
        .ok_or_else(|| AmmError::MathOverflow)?;
    return Ok(result);
}
...
pub fn get_delta_amount_quote_unsigned(
    ...
    require!(result <= U256::from(u64::MAX), AmmError::MathOverflow);
    return Ok(result.try_into().map_err(|_| AmmError::TypeCastFailed)?);
}
