...
/// @title The interface for the Pump Reward System
/// @notice The Pump Reward manages fee configuration, cashback rewards, and referral tracking
interface IPumpReward {
    // ============ Structs ============
    ...
    /// @notice Emitted when protocol fee is added (from migration or other sources)
    /// @param curve The curve that contributed the protocol fee
    /// @param amount The protocol fee amount added
    event ProtocolFeeAdded(address indexed curve, uint256 amount);
    ...
    /// @notice Returns accumulated protocol fees
    /// @return amount The accumulated protocol fees (in quote tokens)
    function accumulatedProtocolFees() external view returns (uint256 amount);
    ...
    /// @notice Add protocol fee from a curve (e.g., migration fees)
    /// @dev Callable by authorized curves only. Adds to accumulated protocol fees.
    /// @param amount The protocol fee amount in quote token (USDC, 6 decimals)
